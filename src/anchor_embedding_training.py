...
# "data/lexicons/mk-en_lexicon_no_test.txt"
bi_dict_path = os.path.abspath(cwd+'/'+sys.argv[1])
# "data/macedonian/mkwiki.full"
tgt_data_path = os.path.abspath(cwd+'/'+sys.argv[2])
# "data/macedonian/embeddings/W2V_MK_300D_CBOW_aligned.kv"
tgt_embedding_path = os.path.abspath(cwd+'/'+sys.argv[3])
limit = int(sys.argv[4])
# "data/embeddings/EN_300D_CBOW.kv"
src_embedding_path = os.path.abspath(cwd+'/'+sys.argv[5])
train_type = 1 if sys.argv[6].lower().strip() == 'sg' else 0
size = int(sys.argv[7])
max_final_vocab = int(sys.argv[8])

training_corpus = SentenceIterator(tgt_data_path)
# Load a word2vec model stored in the C *text* format.
vectors_src = KeyedVectors.load_word2vec_format(
    datapath(src_embedding_path), binary=False)
